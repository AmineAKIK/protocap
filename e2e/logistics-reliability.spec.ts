...
const LEGACY_KEY = 'lineops.logistics.requests.v8';
const KEY = 'lineops.logistics.requests.v9';
...
test.describe('PR-07 Logistics reliable persistence', () => {
  test('T11/T12/T42: failed creation preserves draft and retry confirms exactly one request', async ({ page }) => {
    ...
    const stored = JSON.parse((await page.evaluate((key) => localStorage.getItem(key), KEY))!) as { requests: { zone: string; comment?: string }[] };
    expect(stored.requests.filter((request) => request.zone === 'Zone retry' && request.comment === 'Brouillon navigateur')).toHaveLength(1);
  ...
  test('T20: missing historical closure stays unknown and causes no repair write', async ({ page }) => {
    ...
    expect(await page.evaluate((legacyKey) => JSON.parse(localStorage.getItem(legacyKey) ?? '[]')[0].completedAt, LEGACY_KEY)).toBeUndefined();
  });

  test('T22/T42/T43: cancellation is confirmed and terminal without horizontal overflow', async ({ page }) => {
    await page.addInitScript((key) => {
      localStorage.setItem(key, JSON.stringify({ schemaVersion: 9, revision: 1, requests: [{
