...
The protected service may advertise an isolated demo only through `PUBLIC_DEMO_URL`, which is the verified demo origin. The public API derives the explicit `/demo` entry URL from that origin. The demo origin is intentionally ShiftGuide-only: `/demo`, `/` and unrelated ProtoCap public routes resolve to `/shiftguide`. The demo-origin guard then establishes the synthetic session automatically, so neither the ProtoCap home nor the protected code screen appears in the public demo journey. Keep `PUBLIC_DEMO_URL` empty until the target origin has been checked against the intended commit/configuration. Outside loopback tests, the value must use HTTPS.

The public-demo browser suite runs protected and demo loopback origins together and verifies the actual cross-origin journey from the protected CTA to the demo cockpit, permanent simulation disclosure, absence of the protected unlock route, accessibility and responsive overflow. It does not prove an external deployment; live URL verification remains a release/deployment action.
