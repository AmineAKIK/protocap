...
The protected service may advertise an isolated demo only through `PUBLIC_DEMO_URL`, which is the verified demo origin. The public API derives the explicit `/demo` entry URL from that origin. The `/demo` route immediately enters ShiftGuide and the demo-origin guard establishes the synthetic session automatically, so the protected code screen is never shown in the public journey. Keep `PUBLIC_DEMO_URL` empty until the target origin has been checked against the intended commit/configuration. Outside loopback tests, the value must use HTTPS.
