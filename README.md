...
The `demo` command injects repository-owned fictitious ShiftGuide fixtures and a scripted Céline provider. It rejects an inherited `DEEPSEEK_API_KEY`, never falls back to DeepSeek, and is not evidence of real provider quality or operational data. The demo origin is scoped to ShiftGuide only: `/demo`, `/` and unrelated public application routes all converge on `/shiftguide`. ShiftGuide then automatically establishes an anonymous synthetic session without a maintainer secret; the protected lock screen is never part of that public journey. A permanent simulation notice remains visible. `PUBLIC_DEMO_URL` is the verified isolated demo origin; the protected service derives the direct `/demo` entry from it and publishes no placeholder or dead URL.
