...
function DocumentHeader({ page }: { page: number }) {
  return (
    <>
      <div className="flex flex-col items-start gap-1 text-[10px] font-semibold text-slate-500 sm:flex-row sm:justify-between sm:gap-4 sm:text-xs">
...
function PageShell({ page, children }: { page: number; children: React.ReactNode }) {
  return (
    <section className={`${pageClass} flex flex-col px-4 py-5 sm:px-[clamp(1.5rem,4vw,2.5rem)] sm:py-7 min-[1200px]:min-h-[1385px] min-[1200px]:px-14`}>
...
function PageTitle({
  ...
  return (
    <div className="mt-6 max-w-4xl">
      ...
      <h2 className="mt-2 text-[clamp(1.75rem,8vw,2.25rem)] font-black leading-[1.08] tracking-tight text-[#163f5b] sm:text-4xl sm:leading-tight">{title}</h2>
...
function ReminderScreen({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_12px_34px_rgba(15,23,42,0.12)]">
      <div className={`${headerTone} flex min-h-[78px] flex-wrap items-start justify-between gap-3 px-4 py-3 text-white sm:px-5 min-[1200px]:items-center`}>
        <div className="flex min-w-0 basis-full items-center gap-3 sm:basis-[20rem] sm:flex-1 min-[1200px]:basis-auto">
      ...
      {tone !== 'success' && (
        <div className="pilot-reminder-context mx-5 mt-4 grid grid-cols-1 rounded-lg border border-slate-200 bg-white px-2 py-1 text-[#0f274b] sm:grid-cols-2 sm:py-2 min-[1200px]:grid-cols-3 min-[1200px]:py-4">
          <div className="flex items-center gap-2 px-3 py-3 min-[1200px]:py-0">
      ...
      <div className="flex flex-wrap items-start justify-between gap-2 border-t border-slate-200 bg-slate-50 px-5 py-2.5 text-[9px] font-medium text-slate-500 sm:items-center sm:text-[10px]">
        <span className="flex basis-full items-center gap-1 sm:basis-auto"><ShieldCheck className="h-3.5 w-3.5" /> Assistant de rappel des prélèvements</span>
        <span className="basis-full sm:basis-auto">{footer}</span>
      </div>
    </div>
  );
}
...
function SampleCard({ number, quantity, unit, label, icon }: { number: number; quantity: string; unit: string; label: string; icon: React.ReactNode }) {
  return (
    <div className="min-w-0 rounded-lg border border-red-100 bg-red-50/70 p-4">
      <div className="flex items-end gap-2 text-[#b90e0e]">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#c70b0b] text-xs font-black text-white">{number}</span>
...
function SuccessButton({ plural = true }: { plural?: boolean }) {
  return (
    <div className="mt-4 flex items-center justify-center gap-2 rounded-lg bg-[#079447] px-4 py-3.5 text-center text-base font-black uppercase text-white shadow-inner sm:text-lg">
      <CheckCircle2 className="h-6 w-6 shrink-0" /> {plural ? 'Prélèvements effectués' : 'Prélèvement effectué'}
    </div>
  );
}
...
export function PilotProposalPage() {
  return (
    <div className="bg-[#eef2f5] px-2 py-6 sm:px-4 sm:py-10 min-[1200px]:px-8">
      <div className="space-y-6 sm:space-y-10">
        <PageShell page={1}>
          ...
          <div className="pilot-summary-grid mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 min-[1200px]:grid-cols-3">
          ...
          <div className="mt-10 grid grid-cols-1 gap-5 min-[1200px]:grid-cols-[1fr_0.9fr]">
            <div className="rounded-xl border border-slate-200 p-6">
              <div className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Aujourd’hui</div>
              <div className="pilot-inline-flow mt-4 flex flex-col items-stretch gap-4 text-[#0f274b] sm:flex-row sm:items-center sm:gap-[clamp(0.5rem,1.5vw,1rem)]">
                <div className="flex-1 rounded-lg bg-slate-100 px-3 py-3 text-center text-sm font-black min-[1200px]:basis-[8.5rem]">Événement S.P.I.</div>
                <ArrowRight className="h-5 w-5 shrink-0 text-slate-400" />
                <div className="flex-[1.25] rounded-lg bg-slate-100 px-3 py-3 text-center text-sm font-black min-[1200px]:basis-[10rem]">Interprétation de la règle</div>
                <ArrowRight className="h-5 w-5 shrink-0 text-slate-400" />
                <div className="flex-1 rounded-lg bg-slate-100 px-3 py-3 text-center text-sm font-black min-[1200px]:basis-[8.5rem]">Prélèvement</div>
              </div>
            </div>
        ...
        <PageShell page={2}>
          ...
          <div className="pilot-summary-grid mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 min-[1200px]:grid-cols-3">
        ...
        <PageShell page={3}>
          ...
          <div className="mt-8 grid grid-cols-1 gap-5 min-[1200px]:grid-cols-2">
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <div key={item} className="flex min-w-0 items-start gap-3 rounded-lg border border-slate-200 px-4 py-3 text-sm leading-6 text-slate-700">
        ...
        <PageShell page={4}>
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 min-[1200px]:grid-cols-[1.45fr_0.55fr]">
        ...
        <PageShell page={5}>
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 min-[1200px]:grid-cols-2">
            <div className="min-w-0">
              <div className="mb-3 flex items-center gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#0f274b] text-sm font-black text-white">1</div>
            ...
            <div className="min-w-0">
              <div className="mb-3 flex items-center gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#07843d] text-sm font-black text-white">2</div>
          ...
          <div className="mt-8 flex items-center justify-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-5 py-5 text-center text-sm font-semibold text-slate-700">
            <Bell className="h-5 w-5 shrink-0 text-[#0f274b]" /> Un rappel actif reste visible jusqu’à sa validation ; le suivant apparaît ensuite.
          </div>
        </PageShell>
        ...
        <PageShell page={6}>
            ...
            <div className="min-w-0">
              ...
              <ReminderScreen title="Surveillance S.P.I. temporairement indisponible" tone="warning" subtitle="Communication S.P.I. indisponible" event="Communication S.P.I." time="09:16">
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
                  <div className="flex items-center gap-2 text-base font-black text-amber-800"><WifiOff className="h-5 w-5 shrink-0" />S.P.I. et la production ne sont pas affectés.</div>
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 gap-5 min-[1200px]:grid-cols-[1.2fr_0.8fr]">
        ...
        <PageShell page={7}>
          ...
          <div className="mt-8 overflow-hidden rounded-xl border border-slate-200">
            <div className="flex flex-wrap items-center justify-between gap-3 bg-[#0f274b] px-5 py-4 text-white">
              <div className="min-w-0">
              ...
              <Database className="h-7 w-7 shrink-0" />
            </div>
        ...
        <PageShell page={8}>
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 min-[1200px]:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
