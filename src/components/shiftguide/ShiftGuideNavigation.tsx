...
interface ShiftGuideNavigationProps {
  loggingOut: boolean;
  onLogout: () => void;
}
...
function LogoutButton({
  ...
  onLogout,
  className,
}: ShiftGuideNavigationProps & { className: string }) {
  return (
    <button
      type="button"
      onClick={onLogout}
      disabled={loggingOut}
      className={`${className} ${navClass(false, true)}`}
    >
      <LogOut size={18} />
    ...
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-24 border-r border-zinc-800 bg-zinc-950 lg:flex lg:flex-col">
      ...
      <div className="border-t border-white/10 p-2">
        <LogoutButton
          ...
          onLogout={onLogout}
          className="flex min-h-14 w-full flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-black transition disabled:opacity-50"
        />
      </div>
    </aside>
  );
    ...
    >
      <div className="flex overflow-x-auto px-1 py-1.5" style={{ scrollbarWidth: 'none' }}>
        ...
        <LogoutButton
          loggingOut={loggingOut}
          onLogout={onLogout}
          className="flex min-h-14 min-w-16 flex-1 flex-col items-center justify-center gap-1 rounded-xl px-2 text-[10px] font-black transition disabled:opacity-50"
        />
      </div>
    </nav>
  );
